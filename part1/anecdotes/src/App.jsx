...
const App = () => {
  ...
  const [votes, getVotes] = useState(Array(8).fill(0))
  console.log(votes)
   
  const [selected, setSelected] = useState(0)

  const incrementVotes = () => {
    const updatedVotes = votes.map((count, index) => {
      if (index == selected) {
        return count+=1
      }
      else {
  ...
  return (
    <div>
      ...
      <div>No of votes: {votes[selected]}</div>
      ...
      <p>With {votes[highestVote]} votes</p>
    </div>
  )
}
